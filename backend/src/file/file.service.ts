...
const filesUrls:string[]= await Promise.all(files.map(async (file)=>{
    await writeFile(`${uploadFolder}/${file.originalname}`,file.buffer)//записываем в папку  aploads фай и его буфер
    return file.originalname//создаём массив из путей к файлу
