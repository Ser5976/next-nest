...
const Poster = dynamic(
  () => import('../../components/page-components/Admin/Poster/Poster'),
  { ssr: false }
);

const AdminPosterPage: NextPageAuth<AdminPosterProps> = ({ productType }) => {
  return (
    <Layout title="Адин панель">
      <Poster productType={productType} />
    </Layout>
  );
};

AdminPosterPage.isOnlyAdmin = true; //только для админа

// подключаем редакс к getStaticProps при помощи wrapper
export const getStaticProps: GetStaticProps<AdminPosterProps> =
